...
def main():
    ...
    plot_message_sizes("data/spotify.csv")
    plt.title("CCDF of Message Sizes")
    plt.legend()
    plt.savefig('../../res/message_sizes.png')
...
def plot_message_sizes(filename: str):
    ...
    #plt.yscale('log')
    plt.plot(bin_edges[1:], ccdf, label=filename.split('/')[1].split('.')[0])
    plt.xlabel("Normalized Packet Length to their maximum value")
    plt.ylabel("CCDF")
    #plt.title("CCDF of Message Sizes for " + filename)
    plt.grid(which='both', linestyle='-', linewidth=0.5)
    #plt.legend()
    # save the plot in res folder
    #plt.savefig("../../res/message_sizes/" + filename.split('/')[1].split('.')[0] + ".png")
